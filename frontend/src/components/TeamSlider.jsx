      ...
      >
        {team.map((member) => (
          <SwiperSlide key={member.name} className="pb-2">
            <AnimatedCard className="h-full rounded-xl p-6 text-center">
              <img
                src={member.image}
                alt={member.name}
                className="mx-auto h-24 w-24 rounded-full border border-slate-200 object-cover dark:border-slate-700"
              />
              <h3 className="mt-5 text-xl font-semibold text-slate-900 dark:text-white">{member.name}</h3>
              ...
              <div className="mt-5 flex items-center justify-center gap-3">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-300 bg-white text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"
